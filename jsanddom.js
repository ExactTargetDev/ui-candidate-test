...
function findDistinctValues(values) {
	...
	return Object.keys(hash).sort().map(function (str) {
...
function doFizzBuzz() {
	// FILL THIS IN
	var soda = Array.apply(0, Array(100)).map(function (x, y) {
			return y + 1;
		}); // [1, 2, 3, ...]
...
function removeFruits(fruits, fruitsToRemove) {
	// FILL THIS IN
	var index = {};
	fruitsToRemove.forEach(function (item) {
		index[item] = true
	});
...
var soda = Array.apply(0, Array(10)).map(function (x, y) {
		return y + 1;
	}); // [1, 2, 3, ...]
...
if (div) {
	...
	button.addEventListener('click', function (e) {
		...
		for (var i = 0; i < list.length; ++i) {
			if (list[i].getAttribute('value') == select.value)
				console.log("name is" + list[i].text);
		}
	});
}
...
if (div) {
	...
	all.addEventListener('click', function (e) {
		...
		for (var k = 0; k < checkboxes.length; ++k) {
			if (checkboxes[k].getAttribute('type') == 'checkbox')
